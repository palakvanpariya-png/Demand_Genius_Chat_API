...
from app.models.chat import JWTAccount
from app.models.chat import JWTAccount, JWToken
...
class JWTGenerator:
    ...
    def retrieve_details_from_token(self, token: str, secret_key: str) -> list[str]:
        try:
            payload = jose_jwt.decode(token=token, key=secret_key, algorithms=[base.JWT_ALGORITHM])
            jwt_account = JWTAccount(user_id=payload["user_id"], tenant_id=payload["tenant_id"])
        ...
        return [jwt_account.user_id, jwt_account.tenant_id]
